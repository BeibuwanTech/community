      ...
      <view class="scroll-y">
        <view class="ft-gap">
          <view class="bg-white">
            ...
            <qui-topic-content
              ref="sun"
              :is-red-packet="thread.isRedPacket"
            ...
            <!-- #ifndef H5-PLAY || MP-WEIXIN-PLAY -->
            <!-- 打赏用户列表 -->
            <view v-if="rewardStatus && thread.type !== 6 && !thread.isRedPacket">
              <qui-person-list
                :type="t.reward"
            ...
            <!-- #endif -->
            <!-- 红包用户列表 -->
            <view v-if="thread.isRedPacket === 1">
              <qui-person-list
                :type="t.reward"
                :person-num="thread.rewardedCount"
                :limit-count="limitShowNum"
          ...
          <view class="comment">
            ...
            <view v-if="posts.length > 0">
              ...
              <view v-for="(list, listIndex) in splitLists" :key="listIndex">
                <view v-for="(item, index) in list" :key="item._jv.id">
                  <qui-topic-comment
                    ...
                    :comment-type="0"
                    :post-id="item._jv.id"
                    :comment-avatar-url="item.user.avatarUrl"
                    ...
                    "
                    @commentJump="commentJump(threadId, item._jv.id)"
                    @imageClick="imageClick"
                    @deleteComment="
                      deleteComment(item._jv.id, '3', item.canHide, item.isDeleted, item)
                    "
                    @replyComment="replyComment(item._jv.id, listIndex * splitSize + index)"
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 6, // 这是主题回复每页数据条数
      splitSize: 6, // 列表分割size
  ...
  computed: {
    ...
    thread() {
      let thread = this.$store.getters['jv/get'](`threads/${this.threadId}`); //修改了const-->let
      // 添加
      if (!!thread && !!thread.firstPost) {
        thread = this.modifyPostContentHtml(thread);
      }
      if (!thread) {
        return {};
      }
  ...
  methods: {
    ...
    //添加
    //处理链接
    modifyString(arr, str, identification) {
      let newContentLink = [];
      if (arr) {
        for (let i = 0; i < arr.length; i++) {
          newContentLink.push(`<a href='${arr[i]}'>${arr[i]}</a>`);
        }
        newContentLink = this.addHref(newContentLink);
        let strToArr = str.split(identification);
        let newContent = [];
        for (let t = 0; t < strToArr.length; t++) {
          if (strToArr[t]) {
    ...
    addHref(htmlArr) {
      let hrefRegular = /(href=('|")http)|(src=('|")http)/g;
      let hrefContent = /\bhref=("|')/g;
      for (let j = 0; j < htmlArr.length; j++) {
        if (htmlArr[j].search(hrefRegular) === -1) {
          let newHref = htmlArr[j].match(hrefContent);
          newHref = newHref[0] + 'https://';
          htmlArr[j] = htmlArr[j].replace(hrefContent, newHref);
        }
      }
      return htmlArr;
    },
    ...
    //修改内容字符串
    modifyPostContentHtml(data) {
      let timeStamp1 = '这是替换标签的字符串';
      let timeStamp2 = '这是替换纯链接文本的字符串';
      if (Object.keys(data).length === 0) return data;
      if (!data.firstPost.contentHtml) return;
      let contentHtml = data.firstPost.contentHtml;
      let reg = /((https?|ftp|file):\/\/([-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|])|(www\.([-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|])\.(com|cn|org)))/g;
      let hrefString = /(<a[^>]+>(.+?)<\/a>)|(<img[^>]+>)/g; //匹配整个a标签或img标签
      let hrefStrArr = contentHtml.match(hrefString);
      if (hrefStrArr) {
        hrefStrArr = this.addHref(hrefStrArr);
        contentHtml = contentHtml.replace(hrefString, timeStamp1);
      }
      let contentLink = contentHtml.match(reg);
      if (contentLink) {
        contentHtml = contentHtml.replace(reg, timeStamp2);
        contentHtml = this.modifyString(contentLink, contentHtml, timeStamp2);
      }
      //将a标签重新换回
      if (hrefStrArr) {
        let strToArr = contentHtml.split(timeStamp1);
        let newContentHtml = [];
        for (let t = 0; t < strToArr.length; t++) {
          if (strToArr[t]) {
        ...
        data.firstPost.contentHtml = newContentHtml;
        return data;
      }
      data.firstPost.contentHtml = contentHtml;
      return data;
    },
    // 采纳事件
    ...
    loadThread() {
      ...
      threadAction
        .then(data => {
          this.modifyPostContentHtml(data); //添加
          // 获取阅读数
          const { viewCount = 0 } = data;
          this.viewCount = viewCount;
          this.initRedPacket(data);
          ...
          if (res.isApproved === 1) {
            if (!res.isComment) {
              ...
              for (let index = 0; index < this.posts.length; index++) {
                ...
                if (element._jv.id === this.commentId) {
                  if (!element.lastThreeComments) {
                    element.lastThreeComments = [];
                  }
                  element.lastThreeComments.unshift(res);
        ...
        data.forEach(item => {
          if (!item.isDeleted) {
            const {
              isDeleted,
              _jv: { id },
              user: { id: userId, avatarUrl, username, isReal, groups },
              canLike,
              isLiked,
              createdAt,
              isApproved,
            ...
            const newPost = {
              isDeleted,
              _jv: { id },
              user: { id: userId, avatarUrl, username, isReal, groups: firstGroup },
              canLike,
              isLiked,
              createdAt,
              isApproved,
    ...
    moreContent(param, thread) {
      console.log('删除', param, thread);
      this.moreCancel();
      if (param.type === '0') {
        this.$;
        uni.redirectTo({
          url: `/topic/post?type=${this.thread.type}&operating=edit&threadId=${this.thread._jv.id}&isredbag=${this.thread.isRedPacket}`,
        });
      } else if (param.type === '2' || param.type === '3') {
