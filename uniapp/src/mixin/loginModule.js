...
module.exports = {
  ...
  methods: {
    ...
    wxh5Login(register = 0, rebind = 0) {
      ...
      // 防止App.vue在onLaunch时跳转，导致user/wechat页面无法触发onload
      uni.setStorageSync('h5_wechat_login', 1);
      this.$store.dispatch('session/wxh5Login');
    },
    /**
     * 获取登录需要的参数
        ...
        .then(res => {
          ...
          if (res && res.data && res.data.errors) {
            ...
            if (res.data.errors[0].status === '403' || res.data.errors[0].status === '422') {
              const title =
                this.i18n.t(`core.${res.data.errors[0].code}`) ||
                this.i18n.t(res.data.errors[0].detail[0]);
              uni.showToast({
                icon: 'none',
                title,
                duration: 2000,
