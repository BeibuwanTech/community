...
export default {
  ...
  computed: {
    ...
    post() {
	  ...
	  this.newContentHtml = this.modifyPostContentHtml(post.contentHtml);
	  //添加end
      let hasFirst = false;
  ...
  methods: {
    ...
    addHref(htmlArr) {
      // console.log("为不带http/https的添加https",htmlArr);
      if (!htmlArr) return;
      let hrefRegular = /href=('|")http/g;
    ...
    //修改内容字符串
    modifyPostContentHtml(contentHtml) {
    ...
    modifyPostCommentHtml(obj) {
      obj.forEach(item => {
        item.contentHtml = this.modifyPostContentHtml(item.contentHtml);
      });
      ...
      this.$store
        ...
        .then(res => {
          if (res.isApproved == 1) {
			...
			res.contentHtml = this.modifyPostContentHtml(res.contentHtml); //添加
    ...
    loadPostComments() {
      ...
      this.loadPostCommentStatus = status.run(() =>
        this.$store.dispatch('jv/get', ['posts', { params }]).then(data => {
		  ...
		  //添加
		  this.modifyPostCommentHtml(this.postComments);
