    ...
    <view class="chat-box">
      ...
      <view class="chat-box__footer">
        <view class="chat-box__footer__msg">
          ...
          ></qui-icon>
          <qui-icon
            name="icon-image chat-box__footer__msg__icon"
            size="40"
            :color="emojiShow ? '#1878F3' : '#7D7979'"
            @click="sending_pictures"
          ></qui-icon>
          <view>
    ...
    sending_pictures() {
      ...
      uni.chooseImage({
        ...
        success: res => {
          ...
          res.tempFilePaths.forEach(tmpFile => {
            http
              .upload('attachments', {
                formData: {
                  type: 1,
              ...
              .then(data => {
                if (data.statusCode === 201) {
                  // console.log(data.data.data.attributes.thumbUrl);
                  if (this.dialogId === '0') {
                    const params = {
                      _jv: {
                        type: 'dialog',
                      },
                      recipient_username: _this.username,
                      message_text: _this.msg ? _this.msg : '发送图片',
                      image_url: data.data.data.attributes.thumbUrl,
                    };
                    // 调用发送消息接口
                    this.$store
                      .dispatch('jv/post', params)
                      .then(arr => {
                        if (arr) {
                          this.msg = '';
                          this.dialogId = arr._jv.id;
                          this.getChatRecord(arr._jv.id);
                        }
                      })
                      .catch(err => {
                        console.log(err);
                    ...
                    const params = {
                      ...
                      dialog_id: this.dialogId,
                      message_text: this.msg ? this.msg : '发送图片',
                      image_url: data.data.data.attributes.thumbUrl,
                    };
                    // 调用发送消息接口
                    this.$store
