<template>
  <qui-page :data-qui-theme="theme" class="favorite">
    <scroll-view
      ...
      @scrolltolower="pullUp"
      refresher-enabled="true"
      :refresher-triggered="refresherTriggered"
      @refresherrefresh="pullDown"
      refresher-background="#f9fafc"
  ...
  methods: {
    ...
    toCompile(num) {
      if (num.type === 5 && num.questionTypeAndMoney.type === 0) {
        uni.redirectTo({
          url: `/pagesplay_h5/reward-to-ask?type=${num.type}&operating=draft&threadId=${num._jv.id}&fromdrafts=${this.fromdrafts}`,
        });
      } else {
        uni.redirectTo({
