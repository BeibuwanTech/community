  ...
  methods: {
          ...
          if (payType === 0) {
            if (broswerType === '0') {
              ...
              this.postThread().then(re => {
                ...
                if (re && re._jv && re._jv.json.data.id) {
                  uni.redirectTo({
                    url: `/topic/index?id=${re._jv.json.data.id}`,
                  });
                }
              });
            ...
            if (res.wallet_pay.result === 'success') {
              ...
              this.postThread().then(r => {
                this.postLoading = false;
                uni.hideLoading();
                if (r && r._jv.json.data.id) {
                  uni.redirectTo({
                    url: `/topic/index?id=${r._jv.json.data.id}`,
                  });
                }
              });
            }
          ...
          if (this.payStatus === 1) {
            if (broswerType === '2') {
              ...
              this.postThread().then(data => {
                ...
                if (data && data._jv.json.data.id) {
                  uni.redirectTo({
                    url: `/topic/index?id=${data._jv.json.data.id}`,
                  });
                }
              });
            } else if (broswerType === '1') {
              this.postThread().then(dat => {
                ...
                if (dat && dat._jv.json.data.id) {
                  uni.redirectTo({
                    url: `/topic/index?id=${dat._jv.json.data.id}`,
                  });
                }
              });
            }
    ...
    delAttachments(id, index) {
      ...
      this.uploadFile.forEach((value, key, item) => {
        ...
        valueCopy.id = id && item.splice(key, 1);
        this.uploadFile.splice(key, 1, valueCopy);
      });
    },
