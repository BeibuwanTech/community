...
export default {
  ...
  async created() {
    setTimeout(() => {
      // FIXME:以下的逻辑不知是做什么的，但是会导致 bug
      // 暂时先打补丁
      this.uploadBeforeList = this.uploadBeforeList.concat(this.filePreview);
      this.uploadList.concat(this.filePreview).forEach(item => {
        if (
          this.uploadList.filter(uploadItem => {
            // 如果存在 id 不同，再执行 push 逻辑
            return get(uploadItem, 'id', 'u') === get(item, 'id', 'i');
          }).length === 0
        ) {
  ...
  methods: {
    ...
    clear(index) {
      // TODO 由于在小程序端uploadList中存在重复数据，所以这块暂时采用filter根据id过滤删除
      const todoDelete = this.uploadList[index];

      this.uploadList.sort(this.compare('order'));
      const beforeUpload = this.uploadList[index];
      this.uploadBeforeList = this.uploadBeforeList.filter(item => item.id !== todoDelete.id);
      this.uploadList = this.uploadList.filter(item => item.id !== todoDelete.id);
      this.numberdata = this.numberdata.filter(item => item.id !== todoDelete.id);
      return beforeUpload;
    },
    ...
    upload(pathUrl, index, length, imgOrder, resolve, reject) {
      ...
      if (_this.loginStatus === 1 || _this.loginStatus === 0) {
        _this.header = {
