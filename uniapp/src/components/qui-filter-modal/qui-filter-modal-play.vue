    ...
    >
      ...
      >
        ...
        <view v-for="(item, index) in filterList" class="filter-modal__content__item" :key="index">
          <view class="filter-modal__content__item-title">
            <text class="filter-modal__content__item-redDot"></text>
            {{ item.title }}
          </view>
          <view
...
export default {
  ...
  methods: {
    confirm() {
      if (!this.filterIsClick) {
        this.filterList[0].data.forEach((val, index) => {
          if (val.value === this.selectedData[0].data.value) {
            this.selectedData[0].data.index = index;
          }
        });
      }
      ...
      this.$emit('confirm', this.selectedData, this.secondLevelId);
    ...
    secondLevelItem(item, index) {
      this.secondLevelId = item.search_ids;
...
.filter-modal__content__item-redDot {
  display: inline-block;
  width: 16rpx;
  height: 16rpx;
