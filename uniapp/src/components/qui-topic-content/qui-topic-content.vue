  ...
  <view class="themeItem" v-else>
    <view class="themeItem__header">
      ...
      <view class="themeItem__header__title">
        ...
        <view class="themeItem__header__title__time">
          ...
          >
            悬赏{{ currentPostData.questionTypeAndMoney.money }}元
          </text>
          <!-- <text
          ...
          <view
            class="themeItem_put"
            v-if="themeType === 5 && currentPostData.questionTypeAndMoney.type === 1"
          >
            {{ i18n.t('home.to') }}
          </view>
          <view
    ...
    <view class="themeItem__content">
      ...
      >
        ...
        <view
          ...
          v-if="
            (themeType === 5 && currentPostData.questionTypeAndMoney.type === 0) ||
        ...
        <view
          ...
          v-if="
            (themeType !== 5 && isRedPacket !== 1) ||
              (themeType === 5 && currentPostData.questionTypeAndMoney.type === 1) ||
              (isRedPacket === 1 && themeType === 1)
          "
        >
          <!-- {{currentPostData.postContent}} -->
      ...
      <view class="themeItem__content__tags" v-if="tags.length > 0">
        <view
          ...
          :key="index"
          @click="tagClick(tag._jv.id, tag.search_ids)"
        >
          {{ tag.name }}
        </view>
      </view>
    ...
    >
      <view class="themeItem__content__notes__text">
        该悬赏于 {{ currentPostData.questionTypeAndMoney.expired_at.substring(0, 10) }}日 截止悬赏
        悬赏金额总计
        <text class="themeItem__content__notes__text__redText">
          {{ currentPostData.questionTypeAndMoney.money }}
        </text>
        元，已分配
        <text class="themeItem__content__notes__text__redText">
          {{
            (
              currentPostData.questionTypeAndMoney.money -
              currentPostData.questionTypeAndMoney.remain_money
            ).toFixed(2)
          }}
        </text>
        元
...
export default {
  ...
  methods: {
    ...
    tagClick(tagId, searchIds) {
      this.childrenTabClick(tagId, searchIds);
