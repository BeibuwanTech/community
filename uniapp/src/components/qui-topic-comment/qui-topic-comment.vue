    ...
    <view class="themeItem">
      ...
      >
        ...
        <view class="themeItem__footer__r">
          <view v-if="isShowAdopt" @click="commentAoptClick" class="footer__r__child">
            <image
        ...
        <view class="themeItem__footer__box">
          <view class="themeItem__header__r">
            ...
            <view v-else @click="commentLikeClick" class="comment-like">
              ...
              <qui-icon v-else name="icon-like" class="like" size="30"></qui-icon>
              <view class="comment-like-count">
                {{ commentLikeCount == 0 ? '赞' : `赞${commentLikeCount}` }}
              </view>
            </view>
          </view>
          <view
            class="themeItem__header__r"
            v-if="currentPostData.type === 5 && currentPostData.questionTypeAndMoney.type === 0"
          >
            <view
              v-if="isShowAdopt"
              @click="commentAoptClick"
              class="comment-likecopy comment-adopt"
            >
              <image
                src="../../static/h5-play/adopt@2x.png"
                style="position: relative;right: 10rpx;width: 34rpx;height: 34rpx;"
...
export default {
  ...
  computed: {
    ...
    // 是否显示采纳按钮
    isShowAdopt() {
      if (this.currentPostData.user === undefined) {
        return false;
      }
      return +this.currentLoginId === this.currentPostData.user.id;
    },
    t() {
  ...
  methods: {
    ...
    adoptedAmount() {
      ...
      this.amountMoney = this.percentage * 0.01 * this.currentPostData.questionTypeAndMoney.money;
      this.amountMoney = parseFloat(this.amountMoney.toFixed(2));
    },
    // 采纳事件
    commentAoptClick() {
      if (this.item.user.id === this.currentPostData.user.id) {
    ...
    maskAdoptClick(str) {
      if (str === 'determine') {
        ...
        if (this.amountMoney > this.currentPostData.questionTypeAndMoney.remain_money) {
        ...
        this.amountMoney = '';
        this.percentage = '';
        this.$emit('submitadoption', amountMoney, this.postId * 1, this.currentPostData._jv.id * 1);
        this.$refs.adopt.close();
      } else {
        this.$refs.adopt.close();
      }
