    ...
    >
      ...
      <view class="u-scroll-box" :class="{ 'u-tabs-scorll-flex': !isScroll }">
        <view
          ...
          :key="index"
          @tap="clickTab({ index: index, id: item._jv.id, name: item.name, item: item })"
      ...
      <FilterModal :filter-list="filterList" ref="filter" v-show="true"></FilterModal>
      <!-- #endif -->
    </scroll-view>
  </view>
</template>
 ...
 */
// #ifdef H5-PLAY || MP-WEIXIN-PLAY
import FilterModal from '@/components/h5-play/qui-filter-dropdown/qui-filter-dropdown'
// #endif
export default {
  name: 'UTabs',
  // #ifdef H5-PLAY || MP-WEIXIN-PLAY
  ...
  data() {
    return {
      ...
      scrollBarLeft: 15, // 移动bar需要通过translateX()移动的距离
      ...
      oldScrollLeft: 0,
      // #ifdef H5-PLAY || MP-WEIXIN-PLAY
      filterList:[{text:"1234",textID:1},{text:"1234",textID:2}],
      positionTop: 0,
      // #endif
    };
  },
  ...
  mounted() {
    ...
    // #ifdef H5-PLAY
    this.getTopValue();
    this.handleClickOpenList("",this.getTopValue());
    // #endif
  },
  methods: {
    scroll(event) {
