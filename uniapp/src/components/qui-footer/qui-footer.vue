    ...
    <UniPopupQandA ref="clickOpenQandA" type="center">
      <view class="openOfQandA">
        <view class="openOfQandABox">
          <view class="rewardQuestion" @click="jumpToQandA(1)">
            悬赏提问
          </view>
          <view class="appointQuestion" @click="jumpToQandA(2)">
            向指定人提问
          </view>
  ...
  methods: {
    ...
    footerOpen() {
      ...
      if (this.getCategoryId) {
        if (this.getCategoryId === -1) {
          const category = this.$store.getters['jv/get'](`categories/-1`);
          if (!category.canCreateThread && this.getCategoryId !== -1) {
            this.$refs.toast.show({ message: this.i18n.t('home.noPostingPermission') });
          }
        } else {
          const category = this.$store.getters['jv/get'](
            `categories/${this.getCategoryId.split(',')[0]}`,
          );
          if (!category.canCreateThread && this.getCategoryId !== -1) {
            this.$refs.toast.show({ message: this.i18n.t('home.noPostingPermission') });
          }
    ...
    jumpToQandA(type) {
      if (type === 1) {
        // alert("跳转悬赏提问");
        uni.navigateTo({
          url: `/pagesplay_h5/reward-to-ask?type=5&name=select&categoryId=${this.getCategoryId}&categoryIndex=${this.getCategoryIndex}`,
        });
      }
      if (type === 2) {
        uni.navigateTo({
