...
export default {
  ...
  methods: {
    ...
    async toggleTab(dataInfo) {
      ...
      // #ifdef H5
      uni.navigateTo({
        url: `/pages/home/index?categoryId=${dataInfo.id}&searchIds=${dataInfo.item.search_ids}`,
      });
      // #endif
      // 重置列表
      this.isResetList = true;
    ...
    loadCategories() {
      this.$store.dispatch('jv/get', ['categories', {}]).then(data => {
        ...
        if (this.homeCategoryId) {
          ...
          const itemSearchIds = {};
          const itemSearchId = {};
          itemSearchIds.search_ids = 0;
          itemSearchId.search_ids = -1;
          if (this.homeCategoryId === '-1') {
            ...
            this.categories.forEach((item, index) => {
              if (this.homeCategoryId === item._jv.id) {
                const search = {};
                if (this.homeSearchIds) {
                  search.search_ids = this.homeSearchIds;
                } else {
                  search.search_ids = item.search_ids;
                }
