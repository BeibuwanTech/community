    ...
    <main v-loading="articleLoading">
      <div class="container-post">
        ...
        <div v-if="redPacket && redPacket.isRedPacket === 1" class="desc-text">
          该红包贴
          <span>{{ redPacket.condition === 1 ? `点赞${redPacket.likenum}个` : '回复' }}</span>
          就能瓜分红包，红包共<span>{{ redPacket.number }}</span>个，已领取
          <span>{{ redPacket.number - redPacket.remain_number }}</span>个，还剩
          <span>{{ redPacket.remain_number }}</span>个，
          <span>分完即止。</span>
          /*IFTRUE_pay*/
          <img src="../../assets/pay_imgs/redpacket.png" alt="领红包" class="desc-logo">
          /*FITRUE_pay*/
        </div>
        <div v-if="rewardQuestion && rewardQuestion.type === 0" class="desc-text">
          悬赏于<span>{{ rewardQuestion.showTime }}</span>日截止，
          已分配<span>{{ (rewardQuestion.money - rewardQuestion.remain_money).toFixed(2) }}</span>元，
          剩下<span>{{ rewardQuestion.remain_money }}</span>元。
        </div>
        <qa-actions
          v-if="thread.type === 5 && question.type !== 0"
          :question="question || {}"
          :current-user-id="currentUser && currentUser.id ? currentUser.id : ''"
          :question-user-id="
            thread.user && thread.user.id ? thread.user.id : ''
          "
          @answerPublished="getThread()"
          @payForAnswer="showCheckoutCounter = true"
        />
        <topic-reward-list
          :red-status="thread.isRedPacket || 0"
          :author="thread.user || {}"
          :paid-information="paidInformation"
...
<script>
const threadInclude = 'posts.replyUser,threadAudio,user.groups,user,'
...
export default {
  ...
  async asyncData({ params, store }) {
    ...
    try {
      const threadData = await store.dispatch('jv/get', [
        `threads/${params.id}`,
      ...
      return {
        ...
        postId: threadData.firstPost._jv.id
      };
    } catch (error) {
  ...
  computed: {
    ...
    rewardOrPay() {
      ...
      const questionPrice = Object.keys(this.question).length > 0
  ...
  methods: {
    ...
    initReward(data) {
      if (
        data.type === 5
          && data.questionTypeAndMoney
          && data.questionTypeAndMoney.type === 0
      ) {
        this.rewardQuestion = data.questionTypeAndMoney;
        this.rewardQuestion.showTime = data.questionTypeAndMoney.expired_at.substr(0, 10);
    ...
    autoUpdateRewardStatus() {
      if (this.rewardQuestion.remain_number === 0) return clearInterval(this.timer);
      if (this.onUpdateReward) return;
      this.onUpdateReward = true;
      this.$store.dispatch('jv/get', [
        `threads/${this.threadId}`, { params: { include: threadInclude, stopViewCount: 1 }}
      ]).then(data => {
        this.rewardQuestion = data.questionTypeAndMoney;
        this.rewardQuestion.showTime = data.questionTypeAndMoney.expired_at.substr(0, 10);
