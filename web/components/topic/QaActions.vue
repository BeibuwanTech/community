    ...
    <div v-if="showAnswer" class="block answer">
      <div class="header">
        ...
        <div v-show="question.onlooker_number" class="counter">
          ...
          <span
            v-show="question.onlooker_number > 0"
          >, {{ question.onlooker_number + "人已支付" }}</span>
        </div>
      </div>
      <div
        class="answer-text content-html"
        v-html="$xss(question.content_html)"
      />
      <div
    ...
    <div v-if="showAnswerButton" class="block answer-btn">
      ...
      <div class="tip">
        <span v-show="parseFloat(question.price) > 0">
          ...
          <span class="amount">
            ￥{{ (parseFloat(question.price) * siteUserScale).toFixed(2) }}
          </span>
          <span>{{ $t("post.yuan") }}。</span>
        </span>
        <span v-show="question.is_onlooker && siteUserScale > 0">
          ...
          >￥
            {{
              (
                (parseFloat(question.onlooker_unit_price) * siteUserScale) /
                2
              ).toFixed(2)
            }}
          </span>
      ...
      <button @click="$emit('payForAnswer')">
        ...
        <span>{{
          $t("topic.payAmountCanWatch", {
            amount: question.onlooker_unit_price
          })
        }}</span>
      </button>
    </div>
...
export default {
  ...
  computed: {
    ...
    isBeAskedUser() {
      return (
        parseInt(this.question.be_user_id) === parseInt(this.currentUserId)
      );
    },
    showAnswer() {
      if (this.question.is_answer === 0) return false;
        ...
        return !(
          ...
          && this.question.onlooker_price > 0
        );
      }
    },
    showAnswerButton() {
    ...
    showOnLookerButton() {
      return (
        !this.isBeAskedUser
        && !this.isQuestionUser
        && this.question.onlooker_unit_price > 0
        && !this.question.onlookerState
        && this.question.is_answer === 1
      );
    },
    forums() {
