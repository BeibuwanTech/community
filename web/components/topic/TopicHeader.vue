<template>
  <div class="title">
    ...
    <avatar-component v-else :author="author">
      <span v-if="thread.question.type === 0">
        悬赏发布于{{ publishTime(thread.createdAt) }}
      </span>
      <span v-else>
        {{
    ...
    <div class="container-management">
      ...
      /*FITRUE_pay*/
      <div v-if="thread.isApproved === 0" class="checking">审核中</div>
      <el-dropdown
        v-show="
...
export default {
  ...
  watch: {
    thread(val) {
      if (val && val.category) {
        this.categoryId = val.category._jv.id;
      }
    }
  },
  /*FITRUE_pay*/
  methods: {
    handleCommand({ command, item }) {
      if (command === 'toEdit') {
        return this.$router.push(
          `/thread/postpay?type=${this.thread.type}`
          + `&operating=edit`
          + `&threadId=${this.thread._jv.id}`
          + `&categoryId=${this.thread.category._jv.id}`
        );
      }
    ...
    async toCategoryPage() {
      ...
      if (this.search_ids === '') {
        this.search_ids = this.thread.category._jv.id;
      }
      this.$router.push({
        path: `/category/${this.thread.category._jv.id}`,
    ...
    handleData(data) {
      data.forEach((item) => {
        if (this.categoryId === item._jv.id) {
