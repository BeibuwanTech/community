<template>
  <article class="global">
    ...
    <div v-if="unpaid && threadType === 1" class="hide-content-tip">
      {{ $t("pay.contentHide", {percent: (1 - paidInformation.freeWords) * 100}) }}
    </div>
    /*IFTRUE_default*/
    <!-- <nuxt-link
      v-if="category.name"
...
export default {
  ...
  methods: {
    ...
    async toCategoryPage() {
      ...
      if (this.search_ids === '') {
        this.search_ids = this.category._jv.id;
      }
      this.$router.push({
        path: `/category/${this.category._jv.id}`,
    ...
    handleData(data) {
      data.forEach((item) => {
        if (this.category._jv.id === item._jv.id) {
